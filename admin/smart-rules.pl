...
# save the values entered
    ...
    my $sth_insert = $dbh->prepare("INSERT INTO issuingrules (branchcode, categorycode, itemtype, maxissueqty, issuelength, fine, firstremind, chargeperiod, holdallowed) VALUES(?,?,?,?,?,?,?,?,?)");
    my $sth_update=$dbh->prepare("UPDATE issuingrules SET fine=?, firstremind=?, chargeperiod=?, maxissueqty=?, issuelength=?, holdallowed=? WHERE branchcode=? AND categorycode=? AND itemtype=?");
    ...
    if ($res->{total}) {
        $sth_update->execute($fine, $firstremind, $chargeperiod, $maxissueqty, $issuelength, $holdallowed, $br, $bor, $cat);
    } else {
        $sth_insert->execute($br, $bor, $cat, $maxissueqty, $issuelength, $fine, $firstremind, $chargeperiod, $holdallowed);
    }
}
